...
function usePangination({
  amountCatalogPages,
  currentPage,
  onPageChange,
}: TUsePaginationProps): TUsePaginationReturn {
  const arr = Array.from({ length: amountCatalogPages }, (_, i) => i + 1);

  const ranges: number[][] = arr.reduce(
    (acc, page, index) => {
  ...
  const initRangeIndex = ranges.findIndex((range) =>
    range.includes(currentPage),
  );
  const [currentRangeIndex, setCurrentRangeIndex] = useState(initRangeIndex);
  const currentRange = ranges[currentRangeIndex];

  const prevPage =
    currentRange && currentRange[0] > 1 ? currentRange[0] - 1 : null;
  const nextPage =
    currentRange && currentRange[currentRange.length - 1] < amountCatalogPages
      ? currentRange[currentRange.length - 1] + 1
      : null;
  ...
  return { currentPage, currentRange, setPage, prevPage, nextPage };
}
